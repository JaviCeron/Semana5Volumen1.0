...
public class ConexionSQLite extends SQLiteOpenHelper {
    ...
    public boolean InterTradicional(Dto datos){
        boolean estado = true;
        int resultado;
    ...
    //Forma opcional para consultar datos
   public boolean consultaArticulos(Dto datos){
    ...
    public boolean consultarDescripcion(Dto datos){
        ...
        try{
            String descripcion = datos.getDescripcion();
            Cursor fila = bd.rawQuery("selec codigo, descripcion, precio from articulos where descripcion='"+descripcion+"'", null);
            if(fila.moveToFirst()){
                datos.setCodigo(Integer.parseInt(fila.getString(0)));
                datos.setDescripcion(fila.getString(1));
                datos.setPrecio(Double.parseDouble(fila.getString(2)));
    ...
    public boolean bajaCodigo(final Context context, final Dto datos){
        ...
        try{
            int codigo = datos.getCodigo();
            Cursor fila = bd().rawQuery("selec * from articulos where codigo="+codigo, null);
            if (fila.moveToFirst()){
                datos.setCodigo(Integer.parseInt(fila.getString(0)));
                datos.setDescripcion(fila.getString(1));
                datos.setPrecio(Double.parseDouble(fila.getString(2)));
                ...
                Toast.makeText(context, "No hay resultados encontrados para la busqueda especifica.", Toast.LENGTH_SHORT).show();
            }
        }catch (Exception e){
            estadoDelete = false;
            Log.e("error.",e.toString());
